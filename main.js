...
//  BARRA DE INPUTS
// ══════════════════════════════════════════════════
function colorizeInputBar(raw) {
  if (!raw) return '<span style="color:var(--muted)">—</span>';

  // Parsear tokens del string de input y colorearlos
  return raw
    // Modificadores con +
    .replace(/LT\s*\+\s*/g,  '<span class="xbox-LT">LT</span><span style="color:#555">+</span>')
    .replace(/LB\s*\+\s*/g,  '<span class="xbox-LB">LB</span><span style="color:#555">+</span>')
    .replace(/RT\s*\+\s*/g,  '<span class="xbox-RT">RT</span><span style="color:#555">+</span>')
    .replace(/RB\s*\+\s*/g,  '<span class="xbox-RB">RB</span><span style="color:#555">+</span>')
    // Botones de cara
    .replace(/\[A\]/g, '<span class="xbox-A">[A]</span>')
    .replace(/\[B\]/g, '<span class="xbox-B">[B]</span>')
    .replace(/\[X\]/g, '<span class="xbox-X">[X]</span>')
    .replace(/\[Y\]/g, '<span class="xbox-Y">[Y]</span>')
    // Triggers/bumpers solos
    .replace(/\bLT\b/g, '<span class="xbox-LT">LT</span>')
    .replace(/\bLB\b/g, '<span class="xbox-LB">LB</span>')
    .replace(/\bRT\b/g, '<span class="xbox-RT">RT</span>')
    .replace(/\bRB\b/g, '<span class="xbox-RB">RB</span>')
    // Stick
    .replace(/LS[↑↓←→]/g, m => `<span class="xbox-LS">${m}</span>`)
    .replace(/\bLS\b/g, '<span class="xbox-LS">LS</span>');
}
...
function renderInputBar() {
  ...
  if (combo.length === 0) {
    bar.innerHTML = '<span class="input-bar-empty">Los inputs del combo aparecerán aquí</span>';
    return;
  }
